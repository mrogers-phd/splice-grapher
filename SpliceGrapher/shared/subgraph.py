...
def getGraphDepths(depths, graph):
    ...
    while minpos > 0 and depths[minpos] > 0:
        minpos -= 1
    while maxpos < len(depths) and depths[maxpos] > 0:
        maxpos += 1
    ...
    for f in features:
        ...
        if smallFeature:
            ...
            for n in nodes:
                if n == f.exon:
                    continue
                if f.minpos > n.maxpos and f.minpos - n.maxpos < overlapSize:
                    evidence = n
                if f.maxpos > n.minpos and n.minpos - f.maxpos < overlapSize:
                    evidence = n
                if evidence:
                    break
        ...
        for c in clusters:
            ...
            if featureDepth < minDepth:
                continue

            result.setdefault(c,set())
            # Cases split out to make them more readable:
            if f.minpos < c.minpos and c.maxpos < f.maxpos:
                # Feature contains cluster
                result[c].add(f.exon)
            elif c.minpos < f.minpos and f.maxpos < c.maxpos:
                # Cluster contains feature
                result[c].add(f.exon)
            elif (f.minpos+overlapSize) <= c.maxpos and c.minpos <= (f.maxpos-overlapSize):
                # Cluster overlaps feature by minimum amount
                result[c].add(f.exon)
...
def makeSupportedGraph(graph, clusters, junctions, **args):
    ...
    if verbose:
        sys.stderr.write('Finding supported isoforms in %s\n' % graph.getName())
    ...
    if not supportedNodeDict:
        if verbose:
            sys.stderr.write('  ** no RNA-Seq-supported nodes for %s\n' % graph.getName())
        return
    ...
    for c in supportedNodeDict:
        for n in supportedNodeDict[c]:
            if len(n.isoformSet) == 1:
                if verbose:
                    sys.stderr.write('  isoform %s supported by node %s\n' % (n.isoformString(), n))
    ...
    if not isoforms:
        if verbose:
            sys.stderr.write('  no isoforms were uniquely represented in %s\n' % graph.getName())
        return

    sortedIso = sorted(list(isoforms))
    if verbose:
        sys.stderr.write('  %d isoforms identified for %s:\n  %s\n' % (len(isoforms), graph.getName(), '\n  '.join(sortedIso)))
